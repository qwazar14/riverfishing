 ...
 * bone chain. The chain is what makes the whip look right: {@link RodItemRenderer} spreads this
 * across the joints with the same tip-heavy weights the bend uses, so the tip travels several times
 * further than the butt without any extra state here.
 *
 * <p>Client-side and local-player only — this is how the rod feels in YOUR hands, and nothing about
 * it needs to agree with the server or with what other players see.
 */
public final class RodPhysics {
    ...
    /**
     * Impulse per unit of tension jump, degrees/second of spring velocity. A run or head-shake
     * arrives as a STEP in the synced tension; multiplying the step by this and slamming it into the
     * spring is what makes the rod visibly YANK the moment the fish hits — and the spring's own
     * overshoot supplies the recovery wobble for free. Replaces the old eased course-lean display
     * (§fight-course), which slid the whole pose over with no life in it.
     */
    public static float JERK_GAIN = 220f;
    /** Sustained drag while the fish runs, degrees/s² at full tension — holds the rod pulled over. */
    ...
    public static void update() {
        ...
        // a step up in synced tension is a jerk and kicks the spring, a running fish keeps a steady
