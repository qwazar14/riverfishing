...
public class CullScreen extends Screen {
    ...
    public void extractRenderState(GuiGraphicsExtractor g, int mouseX, int mouseY, float partialTick) {
        ...
        for (int i = 0; i < rowsShown(); i++) {
            ...
            g.text(font, line, cx - w / 2 + 6, y + 3, 0xFFFFFFFF, false);
            if (sel) {
        ...
        // Pager: arrows either side of "range / of", greyed at the ends. Only drawn when there IS a
    ...
    private void drawArrow(GuiGraphicsExtractor g, int x, int y, String glyph, boolean live, int mouseX, int mouseY) {
        ...
        g.centeredText(font, Component.literal(glyph), x + BTN_W / 2, y + 2, live ? 0xFFE8E4D0 : 0xFF555555);
    }
    ...
    public boolean mouseScrolled(double mx, double my, double dx, double dy) {
        // The wheel turns pages too — the row-at-a-time scroll it used to do made the pager and the
        // wheel disagree about where you were.
        if (pages() > 1) {
