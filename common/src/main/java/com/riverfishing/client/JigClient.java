...
public final class JigClient {
    ...
    public static void accept(JigGaugePacket p) {
        ...
        if (hit) {
            ...
            mc.player.playSound(SoundEvents.ITEM_BREAK.value(), 0.5f, 0.7f);   // 26.x: a Holder here
        }
    }
    ...
    private static void drawBig(GuiGraphicsExtractor g, Minecraft mc, Component text, int cx, int y, float scale, int rgb) {
        g.pose().pushMatrix();
        g.pose().translate(cx, y);
        g.pose().scale(scale, scale);
        g.centeredText(mc.font, text, 0, 0, 0xFF000000 | rgb);
        g.pose().popMatrix();
    }
}
