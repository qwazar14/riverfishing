...
public class TyingViseScreen extends AbstractContainerScreen<TyingViseMenu> {
    ...
    public TyingViseScreen(TyingViseMenu menu, Inventory inv, Component title) {
        super(menu, inv, title, 248, 254);   // §26.1: the size goes through the super ctor
    ...
    public void extractBackground(GuiGraphicsExtractor g, int mouseX, int mouseY, float pt) {
        ...
        g.text(font, title, leftPos + 10, topPos + 8, GuiStyle.TEXT, false);
        ...
        g.text(font, line1, cx, ty, GuiStyle.TEXT, false);
        g.text(font, Component.translatable("tooltip.riverfishing.tied_size", a.sizeMm(),
    ...
    private void button(GuiGraphicsExtractor g, int x, int y, int w, String label, boolean on) {
        g.fill(x, y, x + w, y + 12, on ? GuiStyle.SLOT_BG : GuiStyle.GHOST);
        g.fill(x + 1, y + 1, x + w - 1, y + 11, on ? GuiStyle.SLOT_HI : GuiStyle.GHOST);
        int tw = font.width(label);
        g.text(font, label, x + (w - tw) / 2, y + 2, on ? 0xFFF4E8C8 : 0xFF6E5A3C, false);
    }
    ...
    public void extractRenderState(GuiGraphicsExtractor g, int mouseX, int mouseY, float pt) {
        ...
        if (p > 0) {
            ...
            g.setTooltipForNextFrame(font, Component.literal(name + need), mouseX, mouseY);
        }
    }
}
