...
public final class FlyCastClient {
    ...
    private static void renderLine(GuiGraphicsExtractor g, Minecraft mc, int screenW, int screenH) {
        ...
        if (pop > 0.02f) {
            g.pose().pushMatrix();
            g.pose().translate(cx, y + 4);
            g.pose().scale(1f + 0.7f * pop, 1f + 0.7f * pop);
            g.centeredText(mc.font, text, 0, -4, col);
            g.pose().popMatrix();
        } else {
