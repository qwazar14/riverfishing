...
public final class FlyCastClient {
    ...
    public static void render(GuiGraphics g, int screenW, int screenH, float partialTick) {
        ...
        if (beats == 0 && hitNanos < 0) {   // §fly-key: the one line that teaches the cast
            g.drawCenteredString(mc.font, Component.translatable(mode == 1 ? "gui.riverfishing.jig_hint" : "gui.riverfishing.fly_hint"), screenW / 2, y + FH + 8, 0xFFB8AE9A);
        } else if (!openLoop) {
            g.drawCenteredString(mc.font, Component.translatable("gui.riverfishing.fly_hint2"), screenW / 2, y + FH + 8, 0xFFB8AE9A);
        }
        g.drawCenteredString(mc.font, Component.translatable(mode == 1 ? "gui.riverfishing.jig_beats" : "gui.riverfishing.fly_beats", beats),
