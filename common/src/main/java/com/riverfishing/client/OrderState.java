...
public final class OrderState {
    private static final ResourceLocation PANEL = RiverFishing.id("textures/gui/order_panel.png");
    /** The sign occupies the top 48 rows of a 256x64 texture — see tools/gen_order_panel.py. */
    private static final int W = 256, H = 48;

    private static ResourceLocation species;
    ...
    public static void draw(GuiGraphics g) {
        ...
        g.blit(RiverFishing.id("textures/item/fish/" + species.getPath() + ".png"),
                x + 21, y + 16, 16, 16, 0f, 0f, 16, 16, 16, 16);
        ...
        if (pay > 0) {
            ...
            g.renderFakeItem(em, x + 63, y + 16);
            g.renderItemDecorations(font, em, x + 63, y + 16, String.valueOf(pay));
        }
     ...
     */
    private static void engraveCentered(GuiGraphics g, Component c, int cx, int y) {
        engraveCentered(g, c.getVisualOrderText(), cx, y);
    }

    private static void engraveCentered(GuiGraphics g, net.minecraft.util.FormattedCharSequence c, int cx, int y) {
        var font = Minecraft.getInstance().font;
        int half = font.width(c) / 2;
        g.drawString(font, c, cx - half + 1, y + 1, RELIEF, false);
        g.drawString(font, c, cx - half, y, INK, false);
    }
