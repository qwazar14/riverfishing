...
public final class TyingCanvas {
    ...
    public void draw(GuiGraphicsExtractor g, Font font, int left, int top, int mouseX, int mouseY, TackleStationMenu menu) {
    ...
    public void paletteTooltip(GuiGraphicsExtractor g, Font font, int left, int top, int mouseX, int mouseY, TackleStationMenu menu) {
        if (buttonAt(left, top, mouseX, mouseY) == 3) { g.setComponentTooltipForNextFrame(font, bill(menu), mouseX, mouseY); return; }
    ...
    private static void button(GuiGraphicsExtractor g, Font font, int x, int y, int w, int h, String label, boolean on) {
        g.fill(x, y, x + w, y + h, on ? 0xFF6e5a3a : 0xFF2a241c);
        g.fill(x + 1, y + 1, x + w - 1, y + h - 1, on ? 0xFF8a7248 : 0xFF3a3227);
        g.text(font, label, x + (w - font.width(label)) / 2, y + (h - 8) / 2 + 1, on ? 0xFFFFE6B0 : 0xFF6E5A3C, false);
    }
