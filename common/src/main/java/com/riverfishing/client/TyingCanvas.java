...
public final class TyingCanvas {
    ...
    public void paletteTooltip(GuiGraphics g, Font font, int left, int top, int mouseX, int mouseY) {
    ...
    private static void button(GuiGraphics g, Font font, int x, int y, int w, String label, boolean on) { button(g, font, x, y, w, label, on, 12); }

    private static void button(GuiGraphics g, Font font, int x, int y, int w, String label, boolean on, int h) {
        g.fill(x, y, x + w, y + h, on ? 0xFF6e5a3a : 0xFF2a241c);
        g.fill(x + 1, y + 1, x + w - 1, y + h - 1, on ? 0xFF8a7248 : 0xFF3a3227);
        g.drawString(font, label, x + (w - font.width(label)) / 2, y + (h - 8) / 2, on ? 0xFFFFE6B0 : 0xFF6E5A3C, false);
    }
