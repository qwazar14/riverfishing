...
public final class FishMesh {
    ...
    private static final Map<Identifier, float[][]> SIL = new HashMap<>();
     ...
     */
    private static float[][] silhouette(Identifier species) {
        return SIL.computeIfAbsent(species, id -> {
            try {
                Identifier tex = RiverFishing.id("textures/item/fish/" + id.getPath() + ".png");
    ...
    private static int alpha(NativeImage img, int x, int y) {
        return (img.getPixel(x, y) >>> 24) & 0xFF;
    }
     ...
     */
    public static boolean emit(Matrix4f m, VertexConsumer vc, TextureAtlasSprite sp, Identifier species,
