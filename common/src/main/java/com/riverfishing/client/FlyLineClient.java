...
public final class FlyLineClient {
    ...
    public static void tick(Minecraft mc) {
        if (mc.level == null || !holdsRod(mc) || screenOpen(mc)) {
            rope = null;
            stripWas = false;
            feedWas = false;
            stripHeld = 0;
        ...
        if (biting && !bitingWas) {
            ...
            for (int i = 0; i < 12; i++) {
                mc.level.addParticle(net.minecraft.core.particles.ParticleTypes.SPLASH, fx, fy + 0.05, fz,
                        (mc.level.getRandom().nextDouble() - 0.5) * 0.4, 0.15, (mc.level.getRandom().nextDouble() - 0.5) * 0.4);
            }
            ...
            if (landSpeed > 1.5f) {
                ...
                for (int i = 0; i < n; i++) {
                    mc.level.addParticle(net.minecraft.core.particles.ParticleTypes.SPLASH, rope.x[last], rope.y[last] + 0.05, rope.z[last],
                            (mc.level.getRandom().nextDouble() - 0.5) * 0.3 * landSpeed / 4, 0.05 + landSpeed * 0.02, (mc.level.getRandom().nextDouble() - 0.5) * 0.3 * landSpeed / 4);
                }
        ...
        if (onWater && depth < 0.15 && drag > 0.3
            ...
            if (drag > 0.8) {   // skating hard: a little spray at the fly itself
                mc.level.addParticle(net.minecraft.core.particles.ParticleTypes.SPLASH, fx, sy, fz,
                        (mc.level.getRandom().nextDouble() - 0.5) * 0.1, 0.05, (mc.level.getRandom().nextDouble() - 0.5) * 0.1);
            }
        }
    ...
    /** The tip this frame; the picture is drawn by the pass that owns the frame. */
    private static boolean readTip(Minecraft mc, float pt) {
        if (mc.player == null || mc.level == null || !holdsRod(mc)) return false;
        ...
        if (!(mc.options.getCameraType().isFirstPerson() && RodChain.handLineFresh())) {
            tipPrev = tipNow;
            tipNow = tip;
        }
        if (rope == null) return false;
        // in first person the hand pass draws the whole line off the tip it just drew
        return !(mc.options.getCameraType().isFirstPerson() && RodChain.handLineFresh());
    }

    private static void drawRope(Minecraft mc, com.mojang.blaze3d.vertex.VertexConsumer vc, org.joml.Matrix4f m, org.joml.Matrix3f nrm, float pt) {
        float[] style = RodChain.strandStyle(com.riverfishing.component.LineType.FLY, 1.0);
        var main = mc.player.getMainHandItem();
        var rodStack = main.getItem() instanceof RodItem ? main : mc.player.getOffhandItem();
        if (com.riverfishing.item.RodData.get(rodStack, com.riverfishing.component.ComponentSlot.LINE).getItem() instanceof com.riverfishing.item.LineItem li) {
            style = RodChain.strandStyle(li);
        }
        ...
        for (int i = 1; i < rope.n; i++) {
            ...
            LineRenderer.line(vc, m, nrm, prev, p, c[0], c[1], c[2], leader ? 120 : (int) style[3], style[4]);
            prev = p;
        }
        Vec3 fly = prev;
        LineRenderer.line(vc, m, nrm, fly.add(0, 0.03, 0), fly.add(0, -0.03, 0), 30, 30, 30, 255, style[4]);
        LineRenderer.line(vc, m, nrm, fly.add(0.03, 0, 0), fly.add(-0.03, 0, 0), 30, 30, 30, 255, style[4]);
    }
    ...
    static void render(PoseStack pose, Vec3 cam, float pt) {
        ...
        var vc = buffers.getBuffer(net.minecraft.client.renderer.rendertype.RenderTypes.lines());
        drawRope(mc, vc, pose.last().pose(), pose.last().normal(), pt);
