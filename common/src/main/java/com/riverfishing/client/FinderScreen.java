...
public class FinderScreen extends Screen {
    ...
    public FinderScreen(CompoundTag data) {
        ...
        mapCx = water().getIntOr("x", 0) + 0.5;
        mapCz = water().getIntOr("z", 0) + 0.5;
    ...
    private void renderMap(GuiGraphicsExtractor g, int mouseX, int mouseY) {
        ...
        g.text(this.font, Component.translatable("finder.riverfishing.metres", metres), sx + 2, sy - 12, 0x9940E0B0, false);
        ...
        if (chartProv != 0) {
            ...
            g.text(this.font, region, x0 + w - this.font.width(region) - 6, y0 + 5, 0xCCE8B430, false);
        }
        int sounded = ClientSoundings.sounded();
        Component mapped = Component.translatable("finder.riverfishing.mapped", sounded);
        g.text(this.font, mapped, x0 + w - this.font.width(mapped) - 6, y0 + h - 11, 0x9940E0B0, false);
        if (sounded == 0) {
            int ty = y0 + 6;
            for (var seq : this.font.split(Component.translatable("finder.riverfishing.unsounded"), w - 20)) {
                g.text(this.font, seq, x0 + 10, ty, 0xCC40E0B0, false);
                ty += 11;
            }
        }
    }
    ...
    private void renderDetail(GuiGraphicsExtractor g) {
        ...
        if (!detailBlocked) {
            ...
            float bf = t.getFloatOr("bf", 1f);
            Component bedName = Component.translatable("bed.riverfishing." + bedKey(water().getByteOr("bed", (byte) 0)));
    ...
    private List<Line> sampleLines() {
        ...
        CompoundTag suit = data.getCompoundOrEmpty("suit");
        if (!suit.isEmpty()) {
            out.add(head("finder.riverfishing.suit", fishName(suit.getStringOr("sp", ""))));
            List<Component> lines = suitLines(suit);
            for (int i = 0; i < lines.size(); i++) {
                boolean last = i == lines.size() - 1;
                out.add(new Line(null, lines.get(i), !last ? SAMPLE_TEXT : suit.getFloatOr("fit", 0f) > 0 ? SAMPLE_OK : SAMPLE_BAD));
            }
            out.add(plain(Component.empty()));
        }
        out.add(head("finder.riverfishing.sample"));
        out.add(pairLine("finder.riverfishing.water", Component.translatable("water.riverfishing." + w.getStringOr("type", ""))));
        out.add(pairLine("finder.riverfishing.depth", Component.translatable("finder.riverfishing.metres", w.getIntOr("depth", 0))));
        out.add(pairLine("finder.riverfishing.width", Component.translatable("finder.riverfishing.metres", Math.round(w.getFloatOr("width", 0f)))));
        out.add(pairLine("finder.riverfishing.bed_label", Component.translatable("bed.riverfishing." + bedKey(w.getByteOr("bed", (byte) 0)))));
        if (w.contains("clarity")) {
            out.add(pairLine("finder.riverfishing.clarity", Component.literal(Math.round(w.getFloatOr("clarity", 0f) * 100) + "%")));
        }
        List<Component> groups = new ArrayList<>();
        for (String k : w.getStringOr("groups", "").split(";")) {
        ...
        String prov = w.getStringOr("prov", "");
        if (!prov.isEmpty()) {
        ...
        if (!w.getStringOr("season", "").isEmpty()) {
            out.add(pairLine("finder.riverfishing.season", windowName(w.getStringOr("season", ""), w.getStringOr("sub", ""))));
        }
        ...
        for (String k : w.getStringOr("eco", "").split(";")) {
        ...
        if (!farm.isEmpty()) {
            ...
            for (String s : farm.keySet()) {
                ...
                out.add(plain(Component.translatable(f.getBooleanOr("settled", false) ? "finder.riverfishing.farm_row" : "finder.riverfishing.farm_row_new",
                        fishName(s), f.getIntOr("stock", 0), f.getIntOr("f", 0), f.getIntOr("m", 0), f.getIntOr("fry", 0),
                        f.getStringOr("genome", ""), Math.max(0, f.getIntOr("grow", 0)))));
                if (f.contains("spawn")) {
                    int in = f.getIntOr("in", 0);
                    out.add(plain(in <= 0
                            ? Component.translatable("finder.riverfishing.spawn_now", fishName(s), windowName(f.getStringOr("spawn", ""), f.getStringOr("ssub", "")))
                            : Component.translatable("finder.riverfishing.spawn_row", fishName(s), windowName(f.getStringOr("spawn", ""), f.getStringOr("ssub", "")), in)));
                }
            }
        }
    ...
    private List<Component> suitLines(CompoundTag t) {
        CompoundTag w = water();
        int depth = w.getIntOr("depth", 0);
        float width = w.getFloatOr("width", 0f);
        boolean dOk = depth >= t.getIntOr("dmin", 0) && depth <= t.getIntOr("dmax", 0);
        boolean wOk = width >= t.getFloatOr("wmin", 0f) && width <= t.getFloatOr("wmax", 0f);
        String grp = t.getStringOr("bgrp", "");
        Component climate = !grp.isEmpty() ? Component.translatable("biomegroup.riverfishing." + grp)
                : Component.translatable(t.getFloatOr("bio", 0f) > 0 ? "finder.riverfishing.climate_any" : "finder.riverfishing.climate_none");
        List<Component> out = new ArrayList<>();
        out.add(Component.translatable("finder.riverfishing.suit_factors", fmt(t.getFloatOr("wf", 0f)), fmt(t.getFloatOr("sf", 0f)), climate, fmt(t.getFloatOr("bio", 0f))));
        out.add(Component.translatable("finder.riverfishing.suit_gates", depth, dOk ? "✓" : "✗", t.getIntOr("dmin", 0), t.getIntOr("dmax", 0),
                Math.round(width), wOk ? "✓" : "✗", Math.round(t.getFloatOr("wmin", 0f)),
                t.getFloatOr("wmax", 0f) >= 1000 ? "∞" : String.valueOf(Math.round(t.getFloatOr("wmax", 0f)))));
        float fit = t.getFloatOr("fit", 0f);
        ...
        if (t.contains("native")) {
            last.append(" · ").append(Component.translatable(t.getBooleanOr("native", false) ? "finder.riverfishing.suit_native"
                    : t.getBooleanOr("settled", false) ? "finder.riverfishing.suit_settled" : "finder.riverfishing.suit_new"));
        }
        out.add(last);
        return out;
    }
    ...
    private void renderSample(GuiGraphicsExtractor g) {
        ...
        for (int i = sampleScroll; i < lines.size() && y < y0 + h; i++) {
            Line l = lines.get(i);
            if (l.label() != null) {
                g.text(this.font, l.label(), x, y, SAMPLE_LABEL, false);
                g.text(this.font, l.value(), x + 96, y, l.colour(), false);
                y += 11;
                continue;
            }
            for (var seq : this.font.split(l.value(), w - 12)) {
                g.text(this.font, seq, x, y, l.colour(), false);
                y += 11;
            }
        }
        g.disableScissor();
        if (lines.size() > sampleScroll + 1 && y >= y0 + h) {
            g.text(this.font, Component.translatable("finder.riverfishing.more", lines.size() - sampleScroll),
