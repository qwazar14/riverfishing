...
public class FinderScreen extends Screen {
    ...
    private void renderMap(GuiGraphicsExtractor g, int mouseX, int mouseY) {
        ...
        int deepest = 1;
        for (int i = 0; i < map.size(); i++) deepest = Math.max(deepest, map.getCompoundOrEmpty(i).getIntOr("d", 0));
        for (int i = 0; i < map.size(); i++) {
            CompoundTag t = map.getCompoundOrEmpty(i);
            int px = cx + t.getIntOr("x", 0) * CELL, py = cy + t.getIntOr("z", 0) * CELL;
            ...
            float f = t.getIntOr("d", 0) / (float) deepest;
            ...
            if (t.contains("s")) {
                ...
                if (mouseX >= px - 2 && mouseX < px + CELL + 2 && mouseY >= py - 2 && mouseY < py + CELL + 2) {
                    hover = List.of(Component.translatable("spot.riverfishing." + t.getStringOr("s", "")).getVisualOrderText(),
                            Component.translatable("finder.riverfishing.metres", t.getIntOr("d", 0)).getVisualOrderText());
                }
            }
        }
        ...
        if (map.isEmpty()) {
            for (var seq : this.font.split(Component.translatable("finder.riverfishing.unsounded"), VIEW_W - 20)) {
                g.text(this.font, seq, x0 + 10, y0 + 6, 0xCC40E0B0, false);
                y0 += 11;
            }
        }
        g.text(this.font, Component.translatable("finder.riverfishing.map"),
