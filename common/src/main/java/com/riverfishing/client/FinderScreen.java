    ...
    private void renderMap(GuiGraphicsExtractor g, int mouseX, int mouseY) {
        ...
        if (map.isEmpty()) {
            for (var seq : this.font.split(
                ...
                g.text(this.font, seq, x0 + 10, y0 + VIEW_H / 2 - 6, 0x9940E0B0, false);
                y0 += 11;
            }
            return;
        }
        ...
        for (int i = 0; i < map.size(); i++) {
            ...
            if (t.contains("s")) {
                ...
                if (mouseX >= px - 2 && mouseX < px + CELL + 2 && mouseY >= py - 2 && mouseY < py + CELL + 2) {
                    hover = List.of(Component.translatable("spot.riverfishing." + t.getStringOr("s", ""))
                                    .getVisualOrderText(),
                            Component.translatable("finder.riverfishing.metres", t.getIntOr("d", 0))
                                    .getVisualOrderText());
                }
            }
        }
