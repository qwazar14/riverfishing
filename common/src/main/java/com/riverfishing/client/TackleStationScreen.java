...
public class TackleStationScreen extends AbstractContainerScreen<TackleStationMenu> {
    ...
    public void extractBackground(GuiGraphicsExtractor g, int mouseX, int mouseY, float partialTick) {
        ...
        *///?}
        ItemStack[] ghosts = {
                // §26.1: Registry.get(Identifier) returns Optional<Holder.Reference<T>> now — getValue
                // is the direct lookup (matches JournalScreen.modStack).
                new ItemStack(net.minecraft.core.registries.BuiltInRegistries.ITEM
                        .getValue(com.riverfishing.RiverFishing.id("hook_10"))),
        ...
        for (int i = 0; i < ghosts.length; i++) {
            ...
            if (in.isEmpty()) {
                g.fakeItem(ghosts[i], x + wells[i][0], y + wells[i][1]);
                ...
                g.fill(x + wells[i][0], y + wells[i][1],
                        x + wells[i][0] + 16, y + wells[i][1] + 16, 0x8857493a);
            }
            if (need[i] > 0) {
                boolean short_ = in.getCount() < need[i];
                g.centeredText(font, "×" + need[i], x + wells[i][0] + 8, y + 169,
