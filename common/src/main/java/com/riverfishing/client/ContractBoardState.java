...
public final class ContractBoardState {
    ...
    private static int[] bounds() {
        ...
        int n = board.getList("posts", 10).size();
        int h = HEAD + n * ROW + 4;
    ...
    public static void draw(GuiGraphics g, int mouseX, int mouseY) {
        ...
        ListTag posts = board.getList("posts", 10);
        for (int i = 0; i < posts.size(); i++) {
            CompoundTag t = posts.getCompound(i);
            int ry = y + HEAD + i * ROW;
            ...
            g.drawString(font, Component.translatable("journal.riverfishing.contract_short",
                    ...
                    x + 22, ry + 3, INK, false);
            StringBuilder sb = new StringBuilder(ContractItem.grams(t.getInt("W")));
            ...
            for (int k = 0; k < lines.size() && k < 2; k++) {
                g.drawString(font, lines.get(k), x + 22, ry + 13 + k * 9, INK2, false);
            }
            ItemStack em = new ItemStack(Items.EMERALD);
            g.renderFakeItem(em, x + W - 20, ry + 8);
            g.renderItemDecorations(font, em, x + W - 20, ry + 8, String.valueOf(t.getInt("Em")));
        }
    }
    ...
    public static boolean click(double mx, double my) {
        ...
        if (slot < 0 || slot >= board.getList("posts", 10).size()) return false;
        com.riverfishing.network.ModNetwork.toServer(
                new com.riverfishing.network.ContractTakePacket(board.getInt("vid"), slot));
        Minecraft mc = Minecraft.getInstance();
