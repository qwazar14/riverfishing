...
public final class ContractBoardState {
    ...
    private static Component leaves(CompoundTag t) {
        Minecraft mc = Minecraft.getInstance();
        long time = mc.level == null ? 0 : mc.level.getDayTime();
        long daysLeft = t.getLong("Exp") + 1 - time / 24000L;
    ...
    public static void draw(GuiGraphics g, int mouseX, int mouseY) {
        if (!bind()) return;
        Minecraft mc = Minecraft.getInstance();
        var font = mc.font;
        int[] o = origin();
        ...
        for (int i = 0; i < caption.size(); i++) {   // §o: one line, three when it is a debt
            g.drawString(font, caption.get(i), x + 5, y + 15 + LINE * i, rep < 0 ? DEBT : INK2, false);
        }
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            int fy = ry + rh - FOOT + 2;
            g.drawString(font, leaves(t), x + 22, fy + 4, TAKEN, false);
            if (taken) {
                Component tk = Component.translatable("screen.riverfishing.contract_board.taken");
                g.drawString(font, tk, x + W - 5 - font.width(tk), fy + 4, TAKEN, false);
                g.fill(x, ry, x + W, ry + rh, 0x50E3D6B8);       // greyed: yours already, or was
            } else {
                ItemStack em = new ItemStack(Items.EMERALD);
                g.renderFakeItem(em, x + W - 21, fy);
                g.renderItemDecorations(font, em, x + W - 21, fy, String.valueOf(t.getInt("Em")));
            }
        }
    }
    ...
    public static boolean click(double mx, double my) {
        ...
        for (int i = 0; i < posts.length; i++) {
            ...
            CompoundTag post = board.getList("posts", 10).getCompound(i);
            if (post.getBoolean("taken")) return true;           // a taken post swallows the click
