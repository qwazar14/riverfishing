...
public class BaitTrapBlockEntity extends BlockEntity {
     ...
     */
    private void deplete(ServerLevel server, net.minecraft.resources.ResourceLocation species) {
        if (species == null) return;
        com.riverfishing.fishing.FishingPressureData.get(server).addCatch(
                new net.minecraft.world.level.ChunkPos(worldPosition).toLong(), species.getPath(), server.getGameTime());
    }

    /** The species a fry catch stands for: one draw from the same local pool the real catch uses. */
    private net.minecraft.resources.ResourceLocation pickSmallSpecies(ServerLevel server) {
