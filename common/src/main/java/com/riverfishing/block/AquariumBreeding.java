...
final class AquariumBreeding {
    ...
    static void tick(Level level, AquariumBlockEntity be) {
        ...
        long day = level.getOverworldClockTime() / DAY;
    ...
    static boolean use(Level level, AquariumBlockEntity be, Player player, ItemStack held) {
        ...
        if (held.getItem() instanceof RoeItem) {
            ...
            be.incubate = 0;
            be.sync();
            player.sendOverlayMessage(status(level, be));
            return true;
        }
        ...
        if (food && !be.getFishes().isEmpty()) {
            be.fedUntil = level.getOverworldClockTime() / DAY + 1;
            held.shrink(1);
            be.setChanged();
            level.playSound(null, be.getBlockPos(), SoundEvents.GENERIC_EAT.value(), SoundSource.BLOCKS, 0.5f, 1.4f);
            player.sendOverlayMessage(status(level, be));
            return true;
        }
        if (held.isEmpty()) {
    ...
    private static ItemStack[] pair(AquariumBlockEntity be) {
        for (ItemStack f : be.getFishes()) {
            if (!CatchCard.has(f) || CatchCard.of(f).getByteOr("Sex", (byte) 0) != 0) continue;
            Identifier sp = FishItem.getSpecies(f);
            if (sp == null) continue;
            for (ItemStack m : be.getFishes()) {
                if (m != f && CatchCard.has(m) && CatchCard.of(m).getByteOr("Sex", (byte) 0) == 1
    ...
    private static void say(Player player, String key) {
        player.sendOverlayMessage(msg(key));
    }
}
