...
public final class TackleBoxData {
    ...
    public static NonNullList<ItemStack> load(ItemStack box) {
        ...
        ListTag list = StackNbt.get(box).getListOrEmpty(TAG_ITEMS);
        for (int i = 0; i < list.size(); i++) {
            CompoundTag t = list.getCompoundOrEmpty(i);
            int slot = t.getIntOr("s", 0);
            if (slot < 0 || slot >= size) continue;   // a downgraded box drops what no longer fits
            ItemStack s = com.riverfishing.util.RegistryHelper.loadStack(t.getCompoundOrEmpty("i"));
    ...
    public static void save(ItemStack box, NonNullList<ItemStack> contents) {
        ...
        for (int i = 0; i < contents.size(); i++) {
            ...
            t.putInt("s", i);
            t.put("i", com.riverfishing.util.RegistryHelper.saveStack(s));
            list.add(t);
        }
    ...
    public static int used(ItemStack box) {
        return StackNbt.get(box).getListOrEmpty(TAG_ITEMS).size();
    }
