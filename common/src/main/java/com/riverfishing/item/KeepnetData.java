...
public final class KeepnetData {
    ...
    public static KeepnetData read(ItemStack net) {
        ...
        ListTag list = StackNbt.get(net).getListOrEmpty(TAG_ITEMS);
        for (int i = 0; i < list.size(); i++) {
            ...
            if (!s.isEmpty()) {
                out.add(new Placed(s, t.getIntOr("x", 0), t.getIntOr("y", 0), t.getIntOr("r", 0)));
            }
        }
    ...
    public void write(ItemStack net) {
        ...
        for (Placed p : items) {
            ...
            t.putInt("y", p.y());
            t.putInt("r", p.rot());
            t.put("i", com.riverfishing.util.RegistryHelper.saveStack(p.stack()));
            list.add(t);
        }
