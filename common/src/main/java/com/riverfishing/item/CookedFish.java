...
public final class CookedFish {
    ...
    public static ItemStack cook(ItemStack raw) {
        ...
        out.setCount(1);
        StackNbt.mutate(out, t -> t.putBoolean(TAG_COOKED, true));   // 1.20.1: the food is read off the stack when eaten
        return out;
    }
    ...
    public static FoodProperties food(ItemStack fish) {
        ...
        FoodProperties.Builder b = new FoodProperties.Builder().nutrition(nutrition).saturationMod(prime ? 0.6f : 0.35f);
        ResourceLocation sp = FishItem.getSpecies(fish);
        if (sp != null) {
            for (String[] e : effects(sp.getPath())) {
                ResourceLocation id = new ResourceLocation(e[0]);   // "name" parses as minecraft:name
