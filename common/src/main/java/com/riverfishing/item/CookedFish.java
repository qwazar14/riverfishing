...
public final class CookedFish {
    ...
    public static boolean isCooked(ItemStack stack) {
        return stack.getItem() instanceof FishItem && StackNbt.get(stack).getBooleanOr(TAG_COOKED, false);
    }
    ...
    public static ItemStack cook(ItemStack raw) {
        ...
        // 26.x: nutrition is FOOD, the eating and its effects are CONSUMABLE
        out.set(DataComponents.FOOD, food(out));
        out.set(DataComponents.CONSUMABLE, net.minecraft.world.item.component.Consumables.defaultFood()
                .onConsume(new net.minecraft.world.item.consume_effects.ApplyStatusEffectsConsumeEffect(effectsOf(out))).build());
        // the icon reads the cooked flag off the stack: flag 0
        var cmd = out.get(DataComponents.CUSTOM_MODEL_DATA);
        if (cmd != null) {
            out.set(DataComponents.CUSTOM_MODEL_DATA, new net.minecraft.world.item.component.CustomModelData(
                    cmd.floats(), java.util.List.of(true), cmd.strings(), cmd.colors()));
        }
        return out;
    }

    /** What this cooked fish does when eaten: the nutrition. */
    public static FoodProperties food(ItemStack fish) {
        ...
        return new FoodProperties(nutrition, prime ? 0.6f : 0.35f, false);
    }

    /** ...and the effects: a prime fish gives all of its species', an ordinary one only the harm. */
    public static List<MobEffectInstance> effectsOf(ItemStack fish) {
        ...
        for (String[] e : effects(sp.getPath())) {
            Identifier id = Identifier.parse(e[0]);   // "name" parses as minecraft:name
            Holder<MobEffect> effect = BuiltInRegistries.MOB_EFFECT.get(id).map(h -> (Holder<MobEffect>) h).orElse(null);
            if (effect == null) {
            ...
            out.add(new MobEffectInstance(effect, Integer.parseInt(e[2]) * 20, Integer.parseInt(e[1])));
        }
        return out;
    }
