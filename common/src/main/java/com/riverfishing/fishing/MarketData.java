...
public final class MarketData extends SavedData {
    ...
    public static MarketData get(ServerLevel level) {
        ...
        return overworld.getDataStorage().computeIfAbsent(MarketData::load, MarketData::new, NAME);
    }
    ...
    public static MarketData load(CompoundTag tag) {
        ...
        CompoundTag g = tag.getCompound("Glut");
        for (String k : g.getAllKeys()) d.glut.put(k, g.getDouble(k));
        d.lastDay = tag.getLong("LastDay");
        return d;
    }
}
