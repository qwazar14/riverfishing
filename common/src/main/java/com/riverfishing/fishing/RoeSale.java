...
public final class RoeSale {
    ...
    public static int price(Identifier species) {
    ...
    public static void sell(ServerPlayer sp, ItemStack roe) {
        Identifier species = RoeItem.species(roe);
        int each = species == null ? 0 : price(species);
        if (each <= 0) {
            sp.sendOverlayMessage(Component.translatable("message.riverfishing.roe_unwanted",
                    species == null ? roe.getHoverName() : RoeItem.speciesName(species)).withStyle(ChatFormatting.YELLOW));
            return;
        }
        ...
        sp.sendOverlayMessage(Component.translatable("message.riverfishing.roe_sold",
                RoeItem.speciesName(species), em).withStyle(ChatFormatting.GREEN));
        sp.level().playSound(null, sp.blockPosition(), SoundEvents.VILLAGER_YES, SoundSource.PLAYERS, 0.8f, 1.1f);
    }
}
