...
public final class Flow {
    ...
    private static final Map<Long, float[]> CACHE = new LinkedHashMap<>(4096, 0.75f, true) {
        @Override protected boolean removeEldestEntry(Map.Entry<Long, float[]> e) { return size() > 4096; }
    };
    private static Level cachedFor;

    /** Current at (x, y, z) into out[0..2]. Zero for air, still water and every non-river biome. */
    public static void at(Level level, double x, double y, double z, double[] out) {
        out[0] = out[1] = out[2] = 0;
        if (level == null) return;
        BlockPos pos = BlockPos.containing(x, y, z);
        if (level.getFluidState(pos).isEmpty()) return;
        if (level != cachedFor) { CACHE.clear(); cachedFor = level; }
    ...
    private static float[] compute(Level level, BlockPos pos) {
        if (!level.getBiome(pos).is(BiomeTags.IS_RIVER)) return new float[3];
        ...
        if (best < 4) return new float[3];   // a puddle, not a river
        int across = (axis + 2) & 3;
        ...
        int sign = downstream(level, surf, DIRS[axis][0], DIRS[axis][1]);
        dx *= sign; dz *= sign;
    ...
    private static int waterRun(Level level, BlockPos pos, int sx, int sz) {
        int n = 0;
        BlockPos p = pos;
        while (n < AXIS_REACH) {
            p = p.offset(sx, 0, sz);
    ...
    private static int downstream(Level level, BlockPos pos, int sx, int sz) {
