...
public final class WaterUpgrades extends SavedData {
    ...
    public static WaterUpgrades get(ServerLevel level) {
        WaterUpgrades d = level.getDataStorage().computeIfAbsent(WaterUpgrades::load, WaterUpgrades::new, NAME);
        d.day = level.getDayTime() / 24000L;
        return d;
    }
    ...
    public static WaterUpgrades load(CompoundTag tag) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            e.charges = t.getInt("c");
            e.lastDay = t.getLong("d");
            d.entries.put(t.getLong("p"), e);
        }
        return d;
    }
