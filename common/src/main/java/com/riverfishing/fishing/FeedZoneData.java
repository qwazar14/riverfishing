...
public class FeedZoneData extends SavedData {
    ...
    public static FeedZoneData load(CompoundTag tag, net.minecraft.core.HolderLookup.Provider registries) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            ListTag parts = t.getListOrEmpty("parts");
            List<GroundbaitMix.Part> recipe = new ArrayList<>();
            for (int j = 0; j < parts.size(); j++) {
                CompoundTag part = parts.getCompoundOrEmpty(j);
                recipe.add(new GroundbaitMix.Part(part.getStringOr("id", ""), part.getIntOr("n", 0)));
            }
            ...
            Zone z = new Zone(t.getIntOr("x", 0), t.getIntOr("y", 0), t.getIntOr("z", 0),
                    t.getLongOr("fed", 0L), t.getDoubleOr("potency", 0d), mix,
                    t.getIntOr("rgb", mix.rgb()));
