...
public final class ModPotions {
    ...
    /**
     * One brewing-stand row. 1.20.1 has no {@code PotionBrewing.Builder} — Fabric appends to vanilla's static
     * mix list and Forge takes an {@code IBrewingRecipe} — so the two impls hand in their own sink and the
     * recipe itself stays here, said once.
     */
    @FunctionalInterface
    public interface MixSink {
        void addMix(Potion from, Item ingredient, Potion to);
    }

    /**
     * The three mixes, in one place, so both loaders say the same thing (§fish-oil-potion). See the two
     * {@code PlatformHelperImpl.registerBrewing()} for how each loader gets them into the stand.
     *
    ...
    public static void addMixes(MixSink builder) {
        builder.addMix(Potions.WATER, ModItems.FISH_OIL.get(), FISH_OIL.get());
        ...
        for (String sp : OILY) {
            ...
            if (fish != null) builder.addMix(Potions.AWKWARD, fish.get(), FISH_OIL.get());
        }
        builder.addMix(Potions.AWKWARD, ModItems.FISH_OIL.get(), FISH_OIL.get());
        builder.addMix(FISH_OIL.get(), Items.GLOWSTONE_DUST, STRONG_FISH_OIL.get());
    ...
    private static Potion oil(int swim, int mend, int mendAmp, int brace, int braceAmp) {
        return new Potion("fish_oil",
                ...
                new MobEffectInstance(ModEffects.FISH_OIL.get(), swim, 0, false, false, false));
    }
}
