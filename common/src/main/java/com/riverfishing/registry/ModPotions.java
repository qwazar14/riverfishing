 ...
 *
 * <p>§26.1: unchanged from 1.21 apart from the effect names ({@code RESISTANCE}, {@code MINING_FATIGUE}) —
 * brewing is still built in code, not data, so there is no JSON recipe to ship instead.
 */
public final class ModPotions {
     ...
     * being one in 21 (26.2), which hands out {@code asHolder()} instead. Same registry entry either way, so
    ...
    *///?}
    ...
    private static Potion oil(int swim, int mend, int mendAmp, int brace, int braceAmp) {
        return new Potion("fish_oil",
                ...
                new MobEffectInstance(MobEffects.RESISTANCE, brace, braceAmp),
                // ambient/visible/showIcon all false: the cure does its work without a HUD icon or particles.
                new MobEffectInstance(holder(ModEffects.FISH_OIL), swim, 0, false, false, false));
    }
}
