...
/**
 * Cross-loader networking (§multiloader, 1.20.1): Architectury {@link NetworkManager} on the classic
 * {@link FriendlyByteBuf} channel API (1.20.1 has no {@code CustomPacketPayload}/{@code StreamCodec}).
 * Each packet implements {@link RfPacket} (its id + how it writes itself); the receivers decode and hand
 * off — C2S on the server thread, S2C to the client via each packet's own {@code EnvExecutor}-guarded
