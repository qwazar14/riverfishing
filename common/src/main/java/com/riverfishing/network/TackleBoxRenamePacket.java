...
public class TackleBoxRenamePacket implements ModNetwork.RfPacket {
    ...
    public static final ResourceLocation TYPE = RiverFishing.id("tackle_box_rename");
    ...
    public void handleServer(NetworkManager.PacketContext ctx) {
        ...
        String clean = net.minecraft.SharedConstants.filterText(name).trim();
        if (clean.length() > MAX_LENGTH) clean = clean.substring(0, MAX_LENGTH);
        if (clean.isEmpty()) {
            box.resetHoverName();                             // blank field = back to the default name
        } else {
            box.setHoverName(Component.literal(clean));
        }
