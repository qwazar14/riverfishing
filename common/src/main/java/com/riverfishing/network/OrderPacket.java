...
public record OrderPacket(Identifier species, int pay, int base, int tier) implements ModNetwork.RfPacket {
    ...
    private static OrderPacket read(FriendlyByteBuf buf) {
        return new OrderPacket(buf.readIdentifier(), buf.readVarInt(), buf.readVarInt(), buf.readVarInt());
    }
